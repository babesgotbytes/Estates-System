...
if(!isset($_GET['email']) && !isset($_GET['token'])){
    ...
    header("location:StudentforgotpwdPage.html?msg=please enter email to reset password");
}
else{
    ...
    <div class="container container-fluid" style="margin-top: 50px">
        ...
        <form action="StudentResetpwd.php" method="POST">
...
class NewResetPass extends Db_Connect {
    ...
    public function newResetPwd(){
        ...
        $search = "SELECT * FROM PROJECT.student WHERE email=? AND token=? AND token<>'' AND tokenexpire > NOW()";
        $runSearch = $this->connect()->prepare($search);
        if($runSearch->rowCount()>0){

            $newpasshashed=password_hash($this->newpass,PASSWORD_DEFAULT);
            $newtoken = "";
            $updatePass = "UPDATE PROJECT.student SET password =?,token=? WHERE email =?";
            $runUpdate=$this->connect()->prepare($updatePass);
            $passupdate=$runUpdate->execute([$newpasshashed,$newtoken,$this->email]);
            if($passupdate){

                //redirect tot login
                header("location: ../StudentloginPage.php?msg=password changed now login with new password");

            }else{
                // redirect to set new pass
                header('location:../StudentforgotpwdPage.html?msg=unable to change password');
            }
