...
class NewAccount extends Db_Connect{
	...
	public function __construct($username,$email,$pwd,$c_pwd,$time,$regno){
	...
	{
			$errors = new NewAccount($this->username,$this->email,$this->pwd,$this->c_pwd,$this->time,$this->regno);
			...
			if($errors->verifyEmail($this->email)==true){
				...
				if($errors->verifyPassword($this->pwd,$this->c_pwd)==true){
...

					{
								...
								$insert="INSERT INTO PROJECT.student(name,regno,email,pwd,day) VALUES ('$this->username','$this->regno','$this->email','$hashed_pwd','$this->time')";
