...
if(!isset($_GET['email']) && !isset($_GET['token'])){
    ...
    <div class="container container-fluid" style="margin-top: 50px">
        ...
        <form action="StoreKeeperResetpwd.php" method="POST">
...
class NewResetPass extends Db_Connect {
    ...
    public function newResetPwd(){
        ...
        $search = "SELECT * FROM PROJECT.storekeeper WHERE email=? AND token=? AND token<>'' AND tokenexpire > NOW()";
        $runSearch = $this->connect()->prepare($search);
        if($runSearch->rowCount()>0){

            $newpasshashed=password_hash($this->newpass,PASSWORD_DEFAULT);
            $newtoken = "";
            $updatePass = "UPDATE PROJECT.storekeeper SET password =?,token=? WHERE email =?";
            $runUpdate=$this->connect()->prepare($updatePass);
            $passupdate=$runUpdate->execute([$newpasshashed,$newtoken,$this->email]);
            if($passupdate){

                //redirect tot login
                header("location: ../StoreKeeperloginPage.php?msg=password changed now login with new password");

            }else{
                // redirect to set new pass
                header('location:../StoreKeeperforgotpwdPage.html?msg=unable to change password');
            }
