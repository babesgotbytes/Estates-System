...
//attributes to login 
	...
	if(isset($_SESSION['username']) && isset($_SESSION['email'])){
		...
		$query="SELECT * from PROJECT.storekeeper where name=? and email=?";
	...
	class PassReset{
		...
		public function resetPassMailLink($email,$pass,$confim_pwd){
			...
			if(!preg_match($pattern, $email)){
				...
				$query="SELECT * from EUCOSSA.storekeeper where email=?";
				$run_query=$this->connect()->prepare($query);
			...
			}else
			{
				if(password_verify($pass,$hashedpass) != password_verify($confim_pwd,$hashedC_pwd))
				{
					...
					$update="UPDATE EUCOSSA.storekeeper set pwd=? where email=?";//the database and table goes here
					$update_run=$this->connect()->prepare($update);
